...
resource "aws_eks_node_group" "pacificbaking" {
  cluster_name    = aws_eks_cluster.pacificbaking.name
  node_group_name = "pacificbaking"
  node_role_arn   = aws_iam_role.worker-node.arn
  subnet_ids      = aws_subnet.pacificbaking[*].id
